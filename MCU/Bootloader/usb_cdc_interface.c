...
struct Ring_Buffer_t CDC_RB = {CDC_RX_Buffer, 0, 0, sizeof(CDC_RX_Buffer)};
...
int BL_CDC_Get_Char(uint32_t timeout)
{
    ...
    if (timeout)
    {
        ...
        CDC_RB.Read_Index = CDC_RB.Read_Index % CDC_RB.Size;
        CDC_RB.Full_Flag = 0;
        return ch;
    }
 ...
 * @param timeout
 */
int BL_CDC_Get_Chars(uint8_t *buffer, uint32_t count, uint32_t timeout)
{
    uint32_t temp = count;
...
void CDC_Receive_FS_ISR(uint8_t *buff, uint32_t len)
{
    while (len--)
    {
        ...
        CDC_RB.Write_Index = CDC_RB.Write_Index % CDC_RB.Size;
